...
//       image: "uber.png?height=600&width=800",
...
//       image: "/dhvani.png?height=500&width=600",
//       tags: ["React.js", "Firebase", "Fast API", "Python"],
//       demoLink: "",
//       githubLink: "",
//     },
//   ]
...
export default function Projects() {
  ...
  const getVisibleProjects = () => {
    ...
    // On mobile, just show current item and one on each side
    if (isMobile) {
      // Right project
      ...
      // Desktop view - Right side projects (2)
      for (let i = 1; i <= 2; i++) {
        const index = (currentIndex + i) % totalProjects
        result.push({
          ...projects[index],
      ...
      // Desktop view - Left side projects (2)
      for (let i = 1; i <= 2; i++) {
        const index = (currentIndex - i + totalProjects) % totalProjects
        result.push({
          ...projects[index],
  ...
  // Get position-specific styles based on screen size
  const getPositionStyles = (position) => {
    // Mobile styles
    if (isMobile) {
      switch (position) {
  ...
  return (
    <section className="relative w-full max-w-7xl mx-auto py-12 md:py-24 px-4">
      ...
      {/* Carousel container */}
      <div className="relative h-[400px] md:h-[520px] perspective-1000">
        <div className="absolute inset-0 flex items-center justify-center">
          {/* Projects */}
          <div className="relative w-full max-w-xs md:max-w-lg mx-auto h-[350px] md:h-[450px]">
            <AnimatePresence initial={false} mode="popLayout">
              {visibleProjects.map((project) => {
                const styles = getPositionStyles(project.position)
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-4 md:p-6">
                      <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3 text-gray-900 dark:text-white truncate">{project.title}</h3>
                      <p className="text-gray-600 dark:text-gray-300 text-xs md:text-sm mb-3 md:mb-5 line-clamp-2 md:line-clamp-3">{project.description}</p>
                      <div className="flex flex-wrap gap-1 md:gap-2 mb-3 md:mb-5">
                        {project.tags.slice(0, isMobile ? 3 : 4).map((tag, index) => (
                          <span 
                            key={index} 
                            className="px-2 md:px-3 py-1 bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full text-xs font-medium"
                          >
                            {tag}
                          </span>
                        ))}
                        {project.tags.length > (isMobile ? 3 : 4) && (
                          <span className="px-2 md:px-3 py-1 bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full text-xs font-medium">
                            +{project.tags.length - (isMobile ? 3 : 4)}
                          </span>
                        )}
                      </div>
                      <div className="flex space-x-3 md:space-x-5 pt-2 border-t border-gray-100 dark:border-gray-700">
                        <a
                          href={project.demoLink}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center text-xs md:text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center text-xs md:text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors"
                        >
        ...
        {/* Navigation buttons - made larger and more touch-friendly on mobile */}
        <button
          onClick={prevProject}
          className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full p-3 md:p-3 shadow-lg hover:bg-white dark:hover:bg-gray-700 transition-colors border border-gray-200 dark:border-gray-700 touch-manipulation"
          aria-label="Previous project"
        >
          <ChevronLeft className="w-5 h-5 text-gray-700 dark:text-gray-200" />
        </button>
        <button
          onClick={nextProject}
          className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 z-10 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full p-3 md:p-3 shadow-lg hover:bg-white dark:hover:bg-gray-700 transition-colors border border-gray-200 dark:border-gray-700 touch-manipulation"
          aria-label="Next project"
        >
          <ChevronRight className="w-5 h-5 text-gray-700 dark:text-gray-200" />
        </button>
      ...
      <div className="flex flex-col items-center mt-6 md:mt-10 space-y-4 md:space-y-6">
        {/* Autoplay toggle - simplified on mobile */}
        <button
          onClick={() => setIsAutoplay(!isAutoplay)}
          className={clsx(
        ...
        {/* Indicators - better spacing and touch targets for mobile */}
          ...
          {projects.map((_, index) => (
            <button
              ...
              className={clsx(
                "transition-all duration-300",
                index === currentIndex
                  ? "w-6 md:w-8 h-2 bg-blue-600 dark:bg-blue-500 rounded-full"
                  : "w-2 h-2 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 rounded-full",
              )}
              aria-label={`Go to project ${index + 1}`}
            />
